...
AcceptToMemoryPoolWorker(const Config &config, CTxMemPool &pool,
    ...
    //
    //
    // Validate the MAX_TX_SIZE (12MB) is respected
            ...
            if (!test_accept && DoubleSpendProof::IsEnabled()) {
                ...
                if (!entryIt->HasDsp()) {
                    ...
                    try {
                        ...
                        // Prefer pcoinsTip for confirmed outputs.
                        // When there is a mempool conflict, CCoinsViewMemPool may treat the
                        // prevout as unavailable due to the mempool spend, but we still need
                        // the spent output's scriptPubKey/value to create and validate the proof.
                        Coin coin;
                        if (!pcoinsTip->GetCoin(txin.prevout, coin)) {
                            const CCoinsViewMemPool viewMemPool(pcoinsTip.get(), pool);
                            if (!viewMemPool.GetCoin(txin.prevout, coin)) {
                                throw std::runtime_error(strprintf("Could not find coin: %s", txin.prevout.ToString()));
                            }
                        }
                        if (coin.IsSpent()) {
                            throw std::runtime_error(strprintf("Coin is spent: %s", txin.prevout.ToString()));
                        }
...
bool IsInitialBlockDownload() {
    ...
    if (::ChainActive().Tip()->nChainWork < nMinimumChainWork) {
        return false; // Todo: Should be true, but set to false for now while setting up. Change back later
    }
            ...
            if (tx->IsCoinBase()) {
                ...
                if (pushRefSet.size() || requireRefSet.size() || disallowedSiblingRefSet.size() || singletonRefSet.size()) {
                    return state.DoS(
                            ...
                            REJECT_INVALID, "bad-coinbase-txout-contains-induction-op-codes-invalid");
                }
            }
        } 
    }
...
bool CChainState::ActivateBestChain(const Config &config,
    ...
    // RADN maintains a fair degree of expensive-to-calculate internal state
...
static bool ContextualCheckBlock(const CBlock &block, CValidationState &state,
    ...
    // git fetch && git rebase origin/beta1 && ninja &&  sudo systemctl stop radiantd && sudo ninja install  && sudo systemctl restart radiantd && journalctl -f -u radiantd
    return true;
}
...
bool CChainState::LoadBlockIndex(const Config &config,
    ...
    for (const std::pair<const BlockHash, CBlockIndex *> &item : mapBlockIndex) {
        ...
        if (pindex->nHeight == 0 && pindex->hashMerkleRoot.IsNull()) {
            // oops! this block index was never loaded from the disk db! Corruption likely. See RADN issue #244.
