...
void SetupServerArgs() {
    ...
    gArgs.AddArg("-txindex",
                 strprintf("Maintain a full transaction index, used by the "
                           ...
                           DEFAULT_TXINDEX),
                 ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    gArgs.AddArg("-swapindex",
                 strprintf("Maintain a swap advertisement index, used by the "
    ...
    gArgs.AddArg("-uacomment=<cmt>", "Append comment to the user agent string",
                 ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    gArgs.AddArg("-nodeprofile=<profile>",
                 "Select a configuration profile: 'archive' (default), 'agent', or 'mining'. "
                 "'archive' keeps full history and index. 'agent' prunes to minimum size (UTXO focus). "
...
bool AppInitParameterInteraction(Config &config) {
    ...
    // Process -nodeprofile settings before individual args are consumed.
    // User-supplied args (-prune, -txindex) always take precedence over the profile defaults.
    if (gArgs.IsArgSet("-nodeprofile")) {
        std::string profile = gArgs.GetArg("-nodeprofile", "archive");
        if (profile == "agent") {
            // Agent: Minimal pruning (550 MiB), no txindex
            if (!gArgs.IsArgSet("-prune")) {
                gArgs.ForceSetArg("-prune", "550");
            }
            if (!gArgs.IsArgSet("-txindex")) {
                gArgs.ForceSetArg("-txindex", "0");
            }
        } else if (profile == "mining") {
            // Mining: Prune to ~2000 MiB (approx 10,000 blocks @ 200KB avg or full 1MB blocks safety), no txindex
            // 10,000 blocks * 1MB = 10GB worst case.
            // 10,000 blocks * 200KB = 2GB typical case.
            // Let's set it to 4000 MiB (approx 4GB) to be safe for mining needs.
            if (!gArgs.IsArgSet("-prune")) {
                gArgs.ForceSetArg("-prune", "4000");
            }
            if (!gArgs.IsArgSet("-txindex")) {
                gArgs.ForceSetArg("-txindex", "0");
            }
        } else if (profile == "archive") {
            // Archive: No pruning, txindex enabled
            if (!gArgs.IsArgSet("-prune")) {
            ...
            return InitError(strprintf("Unknown node profile: %s", profile));
        }
    }
